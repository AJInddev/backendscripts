<?php
namespace App\Extras\GithubScript;
/*
 * Git.php
 *
 * A PHP git library
...
class GitRepo {
				...
				if ($remote_source) {
					if (isset($reference)) {
						if (!is_dir($reference) || !is_dir($reference.'/.git')) {
							echo('"'.$reference.'" is not a git repository. Cannot use as reference.');
			...
			if ($new_path = realpath($repo_path)) {
				...
				if ($create_new) {
					if ($parent = realpath(dirname($repo_path))) {
						...
						echo('cannot create repository in non-existent directory');
					}
				} else {
    ...
    echo('could not find git dir for '.$this->repo_path.'.');
	}
